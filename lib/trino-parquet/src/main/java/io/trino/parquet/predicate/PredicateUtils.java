...
public final class PredicateUtils
{
            ...
            throws IOException
    {
        ...
        DictionaryPredicateMatcher dictionaryPredicateMatcher = new DictionaryPredicateMatcher(indexPredicate, columnsMetadata, ImmutableSet.copyOf(candidateColumns.get()));
        return new PredicateMatchResult(true, dictionaryPredicateMatcher);
    }
        ...
        for (BlockMetadata block : parquetMetadata.getBlocks(splitStart, splitLength)) {
            for (int i = 0; i < parquetTupleDomains.size(); i++) {
                ...
                if (predicateMatchResult.matchesWithoutDictionary()) {
                    rowGroupInfoBuilder.add(new RowGroupInfo(columnsMetadata, block.fileRowCountOffset(), columnIndex, Optional.ofNullable(predicateMatchResult.dictionaryPredicateMatcher())));
                    break;
                }
            }
        }
    ...
    private record PredicateMatchResult(boolean matchesWithoutDictionary, DictionaryPredicateMatcher dictionaryPredicateMatcher)
    {
        private static final PredicateMatchResult FALSE_WITH_DICT_MATCHING_NOT_NEEDED = new PredicateMatchResult(false, null);
        private static final PredicateMatchResult TRUE_WITH_DICT_MATCHING_NOT_NEEDED = new PredicateMatchResult(true, null);
    }

    public static class DictionaryPredicateMatcher
    {
        private final TupleDomainParquetPredicate indexPredicate;
        private final PrunedBlockMetadata columnsMetadata;
        private final Set<ColumnDescriptor> candidateColumns;

        public DictionaryPredicateMatcher(TupleDomainParquetPredicate indexPredicate, PrunedBlockMetadata columnsMetadata,
        ...
        public boolean dictionaryPredicatesMatch(ColumnDescriptor descriptor, Optional<DictionaryPage> dictionaryPage)
                throws IOException
        {
            ColumnChunkMetadata columnMetaData = columnsMetadata.getColumnChunkMetaData(descriptor);
            if (candidateColumns.contains(descriptor) && isOnlyDictionaryEncodingPages(columnMetaData)) {
                Statistics<?> columnStatistics = columnMetaData.getStatistics();
                boolean nullAllowed = columnStatistics == null || columnStatistics.getNumNulls() != 0;
                return indexPredicate.matches(new DictionaryDescriptor(
                        descriptor,
                        nullAllowed,
                        dictionaryPage));
            }
