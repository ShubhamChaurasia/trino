            ...
            throws IOException
    {
        while (currentRowGroup < rowGroups.size()) {
            ...
            if (currentGroupRowRanges != null) {
                ...
                if (rowCount == 0) {
                    // Continue loop if rowCount is 0
                    continue;
                }
            ...
            boolean dictionaryPredicateMatch = true;
            for (PrimitiveField field : primitiveFields) {
                columnReaders.put(field.getId(), columnReaderFactory.create(field, currentRowGroupMemoryContext));
                if (rowGroupInfo.dictionaryPredicateMatcher().isPresent()) {
                    initializePageReader(field);
                    boolean match = rowGroupInfo.dictionaryPredicateMatcher().get()
                            .dictionaryPredicatesMatch(field.getDescriptor(), columnReaders.get(field.getId()).getDictionaryPage());
                    if (!match) {
                        dictionaryPredicateMatch = false;
                        dictionaryFilteredRowGroups++;
                        break;
                    }
                }
            }
            if (!dictionaryPredicateMatch) {
                continue;
            }
            return true;
        }
